	...
	private function interpretValueDescription( $desc, $property, $pid, &$field, &$must_not, &$filter ) {
		...
		if ( $comparator === SMW_CMP_GRTR || $comparator === SMW_CMP_GEQ ) {
			...
			*/
			...
			if ( $this->options->safeGet( 'query_string.boolean.operators' ) && ( strpos( $value, '+' ) !== false ) ) {
				$must_not = false;
				$value = "-$value";
			}
			...
			if ( $this->options->safeGet( 'query_string.boolean.operators' ) && ( strpos( $value, '+' ) !== false || strpos( $value, '-' ) !== false ) ) {
				$match = $this->fieldMapper->query_string( "$pid.$field", $value );
