...
class ElasticStore extends SQLStore {
	...
	protected function doDataUpdate( SemanticData $semanticData ) {
		...
		if ( $config->dotGet( 'indexer.raw.text', false ) && ( $id = $semanticData->getExtensionData( 'revision.id' ) ) !== null ) {
			$this->doTextUpdate( $subject, $id );
		}
	 ...
	 * @param string $text
	 */
	public function doTextUpdate( DIWikiPage $subject, $id ) {
		...
		$textIndexer->index( $subject, $textIndexer->textFromRevision( $id ) );
		...
		$this->logger->info( '[Store] ElasticStore::doTextUpdate completed (procTime in sec: {procTime})', $context );
	}
